        ...
        BootstrapActions= [
                {
                    ...
                    'ScriptBootstrapAction': {
                            'Path': 's3://landingzonemohit/dependencies/dependencies.sh',
                        }
                }
            ],
...
def livy_task(master_dns):
    ...
    data = {"file": "s3://landingzonemohit/finalscode.py", "className": "com.example.SparkApp","args":["s3://landingzonemohit/config/spark_conf.json","Viewership","Actives"}
    headers = {'Content-Type': 'application/json'}
    #logging.info("querying {host}/batches")
...
#This will poll the livy server every 10 seconds and check if the status of the app is success
def track_statement_progress(master_dns, response_headers):		
    statement_status = ''
    host = 'http://' + master_dns + ':8998'	 #Master dns is address of our cluster
    ...
    while statement_status != 'success':
        ...
        statement_status = statement_response.json()['state']    #get the current status of the cluster and log it
